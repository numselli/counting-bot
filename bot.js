...
bot.on('message', async (message) => {
    ...
    const PREFIX = server?.PREFIX || 'c!';
            
    if (message.content.startsWith(PREFIX)){
        const cmdArguments = (message.content.slice(PREFIX.length).trim()).split(" ")
        return handleCommand(message, cmdArguments, server)
    } 

    if (message.channel.id !== server?.countingChannel) return;
    if (isNaN(message.content)) return deleteMessage(message)
    if (server.lastPerson == message.author.id) {
        deleteMessage(message)
        return message.author.send('You were the last person to count, so you cannot count again!').catch(err => { });
    }

    server.counter++;
    if (server.counter !== parseInt(message.content)) {
        await deleteMessage(message)
        return message.author.send(`That is the wrong number! Next number is: ${"`"}${server.counter}${"`"}.`).catch(err => { });
    }
...
async function handleCommand(message, cmdArguments, server){
    switch(cmdArguments[0]){
        case `setchannel`:
            if (!message.member.permissions.has(['MANAGE_GUILD'])) return sendMessage(message.channel, "In order to use this command you must have the permission ``MANAGE_GUILD``");
            if (server) return serverModel({ serverid: message.guild.id, countingChannel: message.channel.id, PREFIX: 'c!', lastPerson: "0", counter: "0" }).save(err => {
                if (err) {
        ...
        case `unsetchannel`:
            if (!message.member.permissions.has(['MANAGE_GUILD'])) return sendMessage(message.channel, "In order to use this command you must have the permission ``MANAGE_GUILD``");
            if (!server) return sendMessage(message.channel, "This channel is not a counting channel.");
        ...
        case `clear`:
            ...
            if (!server) return serverModel({ serverid: message.guild.id, countingChannel: '0', PREFIX: 'c!', lastPerson: "0", counter: '0' }).save(err => {
                if (err) {
        ...
        break;
        case `count`:
            sendMessage(message.channel, server ? `The count for this server is \`${server.counter}\`, next number is \`${server.counter+1}$\`` : `This channel is not a counting channel.`)
        break;
        case `help`:
        ...
        case `setprefix ${cmdArguments[1]}`:
            ...
            const newPrefix = cmdArguments[1];

            if (!server) return serverModel({ serverid: message.guild.id, countingChannel: '0', PREFIX: newPrefix, lastPerson: "0", counter: '0' }).save(err => {
                if (err) {
